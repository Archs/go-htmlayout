...
func registerWindow(callbacks MsgHandlerMap, windowName string) {
	...
	wproc := syscall.NewCallback(func(hwnd, msg uint32, wparam uintptr, lparam uintptr) uintptr {
		if result, handled := gohl.ProcNoDefault(hwnd, msg, wparam, lparam); handled {
			return result
		}
	...
	wc.Background = 0
	wc.MenuName = nil
	wc.ClassName = syscall.StringToUTF16Ptr(windowName)
	wc.IconSm = 0
...
func createWindow(windowName string) uint32 {
	wcname := syscall.StringToUTF16Ptr(windowName)
	hwnd, errno := CreateWindowEx(
		0,
		wcname,
		syscall.StringToUTF16Ptr("Gohl Test App"),
		0,
		0, 0, 20, 10,
		0, 0, 0, 0)
...
func expectDomError(code gohl.HLDOM_RESULT) {
	if err := recover(); err == nil {
		log.Panic("Expected a DomError but got no error")
	} else if de, ok := err.(*gohl.DomError); !ok {
		...
		log.Panicln("Expected DomError with code")
	}
}
...
func testWithHtml(html string, test func(hwnd uint32)) {
	if !registeredClasses["html"] {
		...
		m[WM_CREATE] = func(hwnd uint32) interface{} {
			...
			if err := gohl.LoadHtml(hwnd, []byte(<-testPages), ""); err != nil {
				log.Panic(err)
			}
			(<-testFuncs)(hwnd)
...
var defaultHandlerMap = MsgHandlerMap{
	WM_CREATE: func(hwnd uint32) interface{} {
		...
		gohl.AttachNotifyHandler(hwnd, notifyHandler)
		gohl.AttachWindowEventHandler(hwnd, windowEventHandler)
		return 0
	},
	...
	WM_CLOSE: func(hwnd uint32) interface{} {
		...
		gohl.DetachWindowEventHandler(hwnd)
		gohl.DetachNotifyHandler(hwnd)
		DestroyWindow(hwnd)
		return nil
	},
...
var notifyHandler = &gohl.NotifyHandler{}

// Window event handler gets first and last chance to process events
var windowEventHandler = &gohl.EventHandler{}

// Tests:

func TestBasicWindow() {
